...
def categorize_transaction(df):
    ...
    for category,keywords in st.session_state.categories.items():
        if category =="Uncategorised" and not keywords:
            continue
...
def add_keywoord_to_category(category,keyword):
    ...
    if(uploaded_file is not None):
        ...
        if df is not None:
            ...
            with tab1:
                ...
                if save_button:
                    for idx,row in edited_df.iterrows():
                        ...
                        add_keywoord_to_category(new_category,details)
                
                st.subheader("Expenses Summary")
                category_totals= st.session_state.debits_df.groupby("Category")["Amount"].sum().reset_index()
                category_totals=category_totals.sort_values(by="Amount", ascending=False)
                print(category_totals.dtypes)
                category_totals["Amount"] = pd.to_numeric(category_totals["Amount"], errors="coerce").fillna(0)
                st.dataframe(
                    category_totals,
                    column_config={
                        "Amount":st.column_config.NumberColumn("Amount", format="%.2f")
            ...
            with tab2:
                ...
                st.write(st.session_state.credits_df)
